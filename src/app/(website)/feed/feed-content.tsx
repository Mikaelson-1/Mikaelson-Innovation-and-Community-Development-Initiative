...
const SocialFeed = () => {
  ...
  useEffect(() => {
    const syncUserToBackend = async () => {
      ...
      try {
        ...
        const userExists = await axios.get(`${BACKEND_URL}/api/v1/users/${userId}`);
        if (!userExists?.data.data?.id) {
          await axios.post(`${BACKEND_URL}/api/v1/users`, { username, email, clerkId: userId });
        }
      } catch (error) {
  ...
  const { data: users } = useQuery<User[]>({
    ...
    queryFn: () => axios.get(`${BACKEND_URL}/api/v1/users/top-contributors`).then(r => r.data.data),
  });

  const { data: challenges } = useQuery<any[]>({
    queryKey: ["challenges"],
    queryFn: () => axios.get(`${BACKEND_URL}/api/v1/challenges`).then(r => r.data.data),
  });
